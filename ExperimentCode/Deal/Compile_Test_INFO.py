...
# 处理编译和测试信息，used to deal compile and test information
class CompileInfo:
    def __init__(self, Log_PATH, model_name, gen_test_PATH):
        ...
        log_fileName = os.path.basename(self.Log_PATH).replace(".java", "_compile.txt")
    ...
    def Compile_INFO(self):
        with open(self.Log_PATH, 'r', encoding='utf-8') as f:
            file_cont = f.read()
        file_cont_list = file_cont.split("########## Test INFO ##########")  # 找到Test INFO之前的信息，即compile info

        # 如果没有编译错误，则直接返回；有编译错误则找到编译错误信息，并写入到Compile_INFO_List中
    ...
    # 处理包含多个块（如错误信息、日志等）的文件，从中提取出最后一个包含错误信息的块
    ...
    # 接收一个包含错误信息的字符串列表，并返回一个包含错误信息的字典列表ERROR_list
    def CompileInfo_Deal(self, errorInfoBlock):
        ERROR_list = []
        # "Error_INFO":报错的信息行, "Error_Line":从报错信息行中抽取出来的行号, "ERROR_Type":从报错信息行中抽取出来的报错信息,
        # "Location":报错处于哪个位置,"TEST_PATH":测试用例的原本路径, Gene_PATH:生成测试用例的路径
        errorInfo_dict = {}         # 临时存储单个错误信息
        TEST_PATH = ""
        Gene_PATH = ""
        singleErrorInfo = []        # 存储单个错误信息的行。
        # PATH = "Prompt2Testing"     # 标识包含路径和报错信息的行
        PATH = "AgentTester"
        added_line = []             # 记录已经处理过的行号，避免重复处理
        for line_i in range(len(errorInfoBlock)):
            if line_i in added_line: continue
            # 如果当前行包含PATH，且不包含[GENERATED PATH]和original test path:，则开始提取错误信息。
    ...
    # 对ERROR_List 进行二次处理，首先将日志文件中的信息xxx_comple.txt文件中，然后根据不同的错误类型提取错误信息，返回字典列表。
    def Call_errorDeal(self):
        # 先将LogINFO中的信息转换到 RepairProcess/xxx_compile.txt文本中
        self.Compile_INFO()

        # "Error_INFO":str, "Error_Line":str, "ERROR_Type":str,"Location":str,"TEST_PATH":str, Gene_PATH:str
        errorInfoBlock = self.divFileIntoBlocks()   # 将转换后的文本文件内容分割成多个错误信息块，每个块对应一个编译错误
        ...
        for error in Error_list:
            ...
            # 这个需要调用java程序完成
                ...
                JarPath = os.path.join(AgentTester_PATH, 'Java_Analyzer', 'src', 'main', 'jarPackage',
                                       'javaparser-core-3.24.7.jar')
                classpath = '.;' + JarPath
                arg1 = self.gen_test_PATH   # 由chatGPT生成的java file
    ...
    # 将一个包含错误信息的列表转换为字典格式：从错误信息列表取出错误信息、错误行号、错误类型、位置和错误符号等信息，并将这些信息存储在一个字典中
