...
# 根据 chatTester_result 从 evosuite 生成的 test method 当中得到一个用于计算coverage
# path = ..\AgentTester v1.1\RepoData\sachin-handiekar_jInstagram.json
def evosuite_first(project_name):
    ...
    for cont in file_cont:
        ...
        # 写入文件中的代码 ../CoverageCal/ProjectResult/Evosuite_first
...
# 从给定的项目结果文件中读取测试结果，并根据这些结果生成新的测试代码文件
def chatTester(project_name, resultDIR="IterateResultDeal"):
    ...
    ChatTester_result = []
    # 遍历final_cont中的每个测试结果，将测试成功的结果路径添加到ChatTester_result列表中
    for cont in final_cont:
        Test_result = cont["Test_result"]
        if Test_result != 1: continue   # 如果测试结果不为1（即测试失败），则跳过该结果
        # 将通过测试的原始路径添加到ChatTester_result列表中，original_path：.../Repos/.../src/test/...**.java
        ChatTester_result.append(cont['original_path'])

        # generated_path：.../ExperimentCode/IterateResultDeal/.../GeneratedTest/**.java
        generated_path = cont['generated_path']
        with open(generated_path, 'r', encoding='utf-8') as f:
            gen_cont = f.read()

        # 从生成文件路径中解析出Method_name和参数数量Parameter_num，然后生成新的文件名sub_file_Name：
        ...
        full_TestDir = os.path.join(current_dir, "ProjectResult", "AgentTester", project_name, TestDir)
        boolean(full_TestDir)
        EST_full_file_path = os.path.join(full_TestDir, TestFileName)
        with open(EST_full_file_path, 'w', encoding='utf-8') as f:
            f.write(Est_code)
